/*mm constructor*/
	...
	;(function(){
				...
				this.play=function(){
							...
							//iso playBay trigger user
								...
								if(mobile){
									...
									nomal = (that.audio.readyState==4||that.audio.ended);
								}
			...
			//play
					if( that.status === "play" ){
						if( that.ctrType === "button" ){
							if( !playFlag ){
								$bnt.addClass("play").removeClass("ready").removeClass("pause");
							}
						}else{
							if( !pauseFlag ){
								$bnt.addClass("pause").removeClass("play").removeClass("ready");
							}
			...
			//stop
						...
						if( !endFlag  && !that.stopQuick ){
							$bnt.addClass("ready").removeClass("play").removeClass("pause");
							stopPlay();
						}
						if( that.stopQuick ){
							$bnt.addClass("ready").removeClass("pause").removeClass("play");
						}
						that.stopTimer();
						that.clrData();
			...
			/*play control*/
				...
				$contain.find(".stop").bind("click",stopPlay);
				function stopPlay(){
					$("#bg_cover").show();
					$(".stop").hide();
					that.stopQuick = true;
